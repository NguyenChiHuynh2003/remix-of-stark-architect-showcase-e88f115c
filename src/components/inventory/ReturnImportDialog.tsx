  ...
  const parseExcelFile = async (file: File) => {
    ...
    try {
      ...
      const returns: ParsedReturn[] = jsonData.map((row) => {
        ...
        const assetId = sanitizeExcelImport(row["Mã tài sản"]);
        const assetName = sanitizeExcelImport(row["Tên tài sản"]);
        const employeeName = sanitizeExcelImport(row["Tên nhân viên"]);
        const returnDate = parseDate(row["Ngày hoàn trả"]);
        const returnCondition = sanitizeExcelImport(row["Tình trạng hoàn trả"]);
        const reusabilityPercentage = parseFloat(row["% Tái sử dụng"]) || null;
        const notes = sanitizeExcelImport(row["Ghi chú"]);
