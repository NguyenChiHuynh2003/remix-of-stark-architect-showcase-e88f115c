...
export const InventoryList = ({ searchItemId, onClearSearchItem }: InventoryListProps) => {
  ...
  useEffect(() => {
    if (!searchItemId || items.length === 0) return;

    // Clear search filter so item is visible
    skipPageResetRef.current = true;
    setSearchQuery("");

    const itemIndex = items.findIndex((i) => i.id === searchItemId);
    ...
    setTimeout(() => {
      ...
      const row = document.querySelector(`[data-inventory-id="${searchItemId}"]`);
      if (row) {
  ...
  // Reset to page 1 when search changes (skip if triggered by search navigation)
  useEffect(() => {
  ...
  const handleExportInventory = async (format: "excel" | "pdf") => {
    const options = {
      ...
      summary: [
        ...
        {
          label: "Tổng tồn kho",
          value: filteredItems.reduce((s, i) => s + Number(i.stock_quantity || 0), 0).toString(),
        },
      ],
    };
    if (format === "excel") {
  ...
  return (
    <div className="space-y-4">
              ...
              {paginatedItems.map((item) => (
                ...
                >
                  ...
                  <TableCell className="text-right">
                    <CurrencyDisplay value={item.wholesale_price} size="sm" hideValue={!canViewValues} />
                  </TableCell>
                  <TableCell className="text-right">
                    <CurrencyDisplay value={item.retail_price} size="sm" hideValue={!canViewValues} />
                  </TableCell>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          <p className="text-sm text-muted-foreground">
            Hiển thị {startIndex + 1}-{Math.min(startIndex + ITEMS_PER_PAGE, filteredItems.length)} /{" "}
      ...
      <InventoryItemDialog open={dialogOpen} onClose={handleDialogClose} editingItem={editingItem} />
    </div>
  );
};
