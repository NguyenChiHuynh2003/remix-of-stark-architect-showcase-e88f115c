...
import { RefreshCw, ArrowLeft, Download, Upload, CheckCircle, Clock, AlertTriangle, Wrench, Package } from "lucide-react";
import {
  Table,
  TableBody,
  TableCell,
...
import { GINReturnList } from "./GINReturnList";
...
export function AssetReturnList() {
  ...
  const exportToExcel = () => {
    const exportData = filteredAllocations.map((allocation) => ({
      ...
      "Loại": allocation.asset_master_data?.asset_type === "equipment" ? "Thiết bị" : "Công cụ",
      "Người sử dụng": allocation.allocated_to_employee?.full_name || (allocation as any).allocated_to_name || "",
  ...
  const getAssetTypeBadge = (assetType: string) => {
    if (assetType === "equipment") {
      return <Badge variant="outline" className="text-blue-600 border-blue-200">Thiết bị</Badge>;
    }
    return <Badge variant="outline" className="text-green-600 border-green-200">Công cụ</Badge>;
  };
                ...
                {loading ? (
                  ...
                  filteredAllocations.map((allocation) => (
                    <TableRow key={allocation.id}>
                      ...
                      <TableCell>
                        {allocation.asset_master_data?.asset_type && getAssetTypeBadge(allocation.asset_master_data.asset_type)}
                      </TableCell>
                      ...
                      <TableCell>{allocation.allocated_to_employee?.full_name || allocation.allocated_to_name || "-"}</TableCell>
                      <TableCell>
