...
export function AssetAllocationList() {
  ...
  const fetchAllocations = async () => {
    try {
      ...
      // Hiển thị tất cả dữ liệu phân bổ (bao gồm dữ liệu cũ)
      // Chỉ giới hạn loại tài sản khi tạo mới phân bổ (trong AssetAllocationDialog)
      ...
      const employeeIds = [
        ...new Set((allocationsData || []).filter((a) => a.allocated_to).map((a) => a.allocated_to)),
      ];
  ...
  const filteredAllocations = allocations.filter((allocation) => {
    return Object.values(allocation).some((value) => String(value).toLowerCase().includes(searchQuery.toLowerCase()));
  });

  // Count statistics
  const uniqueAssets = new Set(allocations.map((a) => a.asset_master_id)).size;
  const uniqueEmployees = new Set(allocations.map((a) => a.allocated_to || a.allocated_to_name)).size;
  ...
  const exportToExcel = () => {
    const exportData = filteredAllocations.map((allocation) => ({
      ...
      "Người sử dụng": allocation.allocated_to_employee?.full_name || allocation.allocated_to_name || "",
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          <Button onClick={exportToExcel} variant="outline" size="sm" disabled={filteredAllocations.length === 0}>
          ...
          <Button onClick={fetchAllocations} variant="outline" size="sm" disabled={loading}>
            ...
            {loading ? (
              ...
              filteredAllocations.map((allocation) => (
                <TableRow key={allocation.id}>
                  ...
                  <TableCell>
                    {allocation.allocated_to_employee?.full_name || allocation.allocated_to_name || "-"}
                  </TableCell>
                  <TableCell>{allocation.purpose}</TableCell>
                  <TableCell>{allocation.projects?.name || "-"}</TableCell>
                  <TableCell>{format(new Date(allocation.allocation_date), "dd/MM/yyyy")}</TableCell>
                  <TableCell>
      ...
      <AssetAllocationDialog open={dialogOpen} onClose={handleCloseDialog} isReturn={false} />

      <AllocationImportDialog open={importDialogOpen} onClose={handleCloseImportDialog} />
    </div>
  );
}
