...
export const OverviewSection = ({ onNavigate }: OverviewSectionProps) => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch total inventory items count from asset_master_data (number of items/products)
      ...
      // Fetch overdue asset allocations
      const { data: overdueAllocations, error: overdueAllocError } = await supabase
        .from("asset_allocations")
        .select("id")
        .eq("status", "overdue");
      if (overdueAllocError) throw overdueAllocError;

      const totalOverdue = (overdueData?.length || 0) + (overdueAllocations?.length || 0);
  ...
  const statsConfig = [
    {
      ...
      color: "text-primary",
      onClick: undefined,
      tooltip: "Số lượng dự án đang trong giai đoạn lập kế hoạch hoặc đang thực hiện",
    },
    {
      ...
      color: "text-accent",
      onClick: undefined,
      tooltip: "Số lượng nhiệm vụ đang ở trạng thái chờ xử lý, chưa được bắt đầu",
    },
    {
      title: "Tổng nhân sự",
      ...
      icon: Users,
      color: "text-primary",
      onClick: onNavigate ? () => onNavigate("hr") : undefined,
      tooltip: "Tổng số nhân viên đang làm việc trong hệ thống",
    },
    {
      ...
      tooltip: "Số lượng loại sản phẩm/vật tư khác nhau trong kho",
    },
    {
      title: "Mục quá hạn",
      ...
      onClick: undefined,
      tooltip: "Tổng số nhiệm vụ và phân bổ tài sản đã quá hạn cần xử lý",
    },
  ];
          ...
          {statsConfig.map((stat, index) => (
            <Tooltip key={index}>
              <TooltipTrigger asChild>
                <Card 
                  className={`border-border shadow-sm hover:shadow-md transition-shadow ${stat.onClick ? 'cursor-pointer hover:border-primary/50' : ''}`}
