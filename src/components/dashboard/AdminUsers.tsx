...
export const AdminUsers = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      let permissionsMap: Record<string, { allowed: string[], canEdit: string[] }> = {};
      
      if (userIds.length > 0) {
        const { data: permsData } = await supabase
          .from("user_permissions" as any)
          .select("user_id, allowed_modules, can_edit_modules")
          .in("user_id", userIds) as any;
        
        permissionsMap = (permsData || []).reduce((acc: Record<string, { allowed: string[], canEdit: string[] }>, p: any) => {
          acc[p.user_id] = {
            allowed: p.allowed_modules || [],
            canEdit: p.can_edit_modules || []
          };
          return acc;
        }, {} as Record<string, { allowed: string[], canEdit: string[] }>);
      }

      // Filter out the hidden admin account (aaa@example.com)
      const filteredUsers = fetchedUsers.filter((user: any) => 
        user.email !== "aaa@example.com"
      );

      const usersWithPermissions = filteredUsers.map((user: any) => {
        const userAllowedModules = permissionsMap[user.id]?.allowed || ["overview"];
        const userCanEditModules = permissionsMap[user.id]?.canEdit || [];
        
        // Check if user has full permissions (all modules with edit access)
        const hasFullPermissions = 
          userAllowedModules.length === availableModules.length &&
          userCanEditModules.length === availableModules.length;
        ...
        return {
          ...
          email: user.email,
          role: user.role || "user",
          allowed_modules: userAllowedModules,
          can_edit_modules: userCanEditModules,
          hasFullPermissions,
        };
      });
  ...
  const toggleModule = (moduleId: string) => {
    setSelectedModules(prev => {
      if (prev.includes(moduleId)) {
        // Also remove from canEditModules when removing from selectedModules
        setCanEditModules(editPrev => editPrev.filter(m => m !== moduleId));
  ...
  const toggleCanEdit = (moduleId: string) => {
    setCanEditModules(prev => 
  ...
  const openPermissionDialog = (user: UserProfile) => {
    ...
    setCanEditModules(user.can_edit_modules || []);
            ...
            <DialogContent>
              ...
              <div className="space-y-4">
                ...
                <Button 
                  ...
                  className="w-full"
                  disabled={!newUserData.email || !newUserData.fullName || !newUserData.password || newUserData.password.length < 6}
                >
                  Tạo tài khoản
                </Button>
              </div>
                ...
                <TableRow key={user.id}>
                  ...
                  <TableCell>
                    <Badge variant="secondary">
                      {(user.role === "admin" || user.hasFullPermissions)
                        ? "Toàn quyền" 
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            <div className="flex gap-2 flex-wrap">
              ...
              <Button variant="outline" size="sm" onClick={selectAllCanEdit}>
                Cấp quyền sử dụng tất cả
              </Button>
              <Button variant="outline" size="sm" onClick={deselectAllCanEdit}>
                Bỏ quyền sử dụng tất cả
              </Button>
            </div>
            
            {/* Header */}
            <div className="grid grid-cols-[1fr,auto,auto] gap-3 items-center px-2 py-1 bg-muted rounded-lg font-medium text-sm">
              <span>Module</span>
              <span className="w-20 text-center">Xem</span>
              <span className="w-24 text-center">Sử dụng chức năng</span>
            </div>
            ...
            <div className="grid grid-cols-1 gap-2 max-h-[300px] overflow-y-auto">
              {availableModules.map((module) => (
                <div 
                  key={module.id} 
                  className="grid grid-cols-[1fr,auto,auto] gap-3 items-center p-2 rounded-lg hover:bg-muted/50 transition-colors"
                >
                  <Label 
                    htmlFor={module.id} 
                  ...
                  <div className="w-24 flex justify-center">
                    <Checkbox
                      id={`edit-${module.id}`}
                      checked={canEditModules.includes(module.id)}
                      disabled={!selectedModules.includes(module.id)}
                      onCheckedChange={() => toggleCanEdit(module.id)}
                    />
                  </div>
                </div>
              ))}
            ...
            <div className="text-sm text-muted-foreground">
              Đã chọn: {selectedModules.length}/{availableModules.length} module xem | {canEditModules.length}/{availableModules.length} module sử dụng chức năng
            </div>
            <Button 
