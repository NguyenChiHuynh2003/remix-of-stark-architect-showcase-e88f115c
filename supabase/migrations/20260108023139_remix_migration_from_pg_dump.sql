...
--
-- Name: can_edit_module(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.can_edit_module(_user_id uuid, _module text) RETURNS boolean
  ...
  SELECT EXISTS (
    ...
    WHERE user_id = _user_id
      AND _module = ANY(can_edit_modules)
  )
$$;


--
-- Name: can_view_module(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.can_view_module(_user_id uuid, _module text) RETURNS boolean
    LANGUAGE sql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
  SELECT EXISTS (
...
CREATE TABLE public.asset_master_data (
    ...
    allocated_quantity numeric DEFAULT 0 NOT NULL,
    warehouse_name text DEFAULT 'KHO CHÍNH'::text,
    opening_quantity numeric DEFAULT 0,
    opening_value numeric DEFAULT 0,
    inbound_quantity numeric DEFAULT 0,
    inbound_value numeric DEFAULT 0,
    outbound_quantity numeric DEFAULT 0,
    outbound_value numeric DEFAULT 0,
...
CREATE TABLE public.employees (
    ...
    employee_card_photos text[] DEFAULT '{}'::text[],
    id_card_photos text[] DEFAULT '{}'::text[],
    certificate_photos text[] DEFAULT '{}'::text[]
);
...
CREATE TABLE public.goods_receipt_notes (
    ...
    receipt_date timestamp with time zone DEFAULT CURRENT_DATE NOT NULL,
    supplier text,
...
--
-- Name: org_chart_positions; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.org_chart_positions (
    ...
    employee_id uuid,
    zone text DEFAULT 'office'::text NOT NULL,
...
--
-- Name: organization_charts; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.tasks (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    completion_percentage numeric DEFAULT 0 NOT NULL,
    CONSTRAINT tasks_completion_percentage_check CHECK (((completion_percentage >= (0)::numeric) AND (completion_percentage <= (100)::numeric))),
...
CREATE TABLE public.user_permissions (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    can_edit_modules text[] DEFAULT '{}'::text[]
);
...
--
-- Name: org_chart_positions org_chart_positions_org_chart_id_position_key_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.org_chart_positions
    ADD CONSTRAINT org_chart_positions_org_chart_id_position_key_key UNIQUE (org_chart_id, position_key);


--
-- Name: org_chart_positions org_chart_positions_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.org_chart_positions
    ADD CONSTRAINT org_chart_positions_pkey PRIMARY KEY (id);


--
-- Name: organization_charts organization_charts_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.organization_charts
    ADD CONSTRAINT organization_charts_pkey PRIMARY KEY (id);
...
--
-- Name: idx_asset_master_data_consumable; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_asset_master_data_consumable ON public.asset_master_data USING btree (is_consumable);


--
-- Name: idx_asset_master_data_type; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_asset_master_data_type ON public.asset_master_data USING btree (asset_type);


--
-- Name: idx_asset_master_data_warehouse; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_asset_master_data_warehouse ON public.asset_master_data USING btree (warehouse_name);
...
--
-- Name: org_chart_positions update_org_chart_positions_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER update_org_chart_positions_updated_at BEFORE UPDATE ON public.org_chart_positions FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();


--
-- Name: organization_charts update_organization_charts_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER update_organization_charts_updated_at BEFORE UPDATE ON public.organization_charts FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
--
-- Name: org_chart_positions org_chart_positions_employee_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.org_chart_positions
    ADD CONSTRAINT org_chart_positions_employee_id_fkey FOREIGN KEY (employee_id) REFERENCES public.employees(id) ON DELETE SET NULL;


--
-- Name: org_chart_positions org_chart_positions_org_chart_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.org_chart_positions
    ADD CONSTRAINT org_chart_positions_org_chart_id_fkey FOREIGN KEY (org_chart_id) REFERENCES public.organization_charts(id) ON DELETE CASCADE;


--
-- Name: organization_charts organization_charts_project_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.organization_charts
    ADD CONSTRAINT organization_charts_project_id_fkey FOREIGN KEY (project_id) REFERENCES public.projects(id) ON DELETE CASCADE;
...
--
-- Name: employees Admins and HR editors can manage employees; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and HR editors can manage employees" ON public.employees USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.has_role(auth.uid(), 'hr_admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'hr'::text))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.has_role(auth.uid(), 'hr_admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'hr'::text)));


--
-- Name: contract_guarantees Admins and accounting editors can manage contract guarantees; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and accounting editors can manage contract guarantees" ON public.contract_guarantees USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.has_role(auth.uid(), 'accountant'::public.app_role) OR public.can_edit_module(auth.uid(), 'accounting'::text))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.has_role(auth.uid(), 'accountant'::public.app_role) OR public.can_edit_module(auth.uid(), 'accounting'::text)));


--
-- Name: contracts Admins and accounting editors can manage contracts; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and accounting editors can manage contracts" ON public.contracts USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.has_role(auth.uid(), 'accountant'::public.app_role) OR public.can_edit_module(auth.uid(), 'accounting'::text))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.has_role(auth.uid(), 'accountant'::public.app_role) OR public.can_edit_module(auth.uid(), 'accounting'::text)));


--
-- Name: accounting_transactions Admins and accounting editors can manage transactions; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and accounting editors can manage transactions" ON public.accounting_transactions USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.has_role(auth.uid(), 'accountant'::public.app_role) OR public.can_edit_module(auth.uid(), 'accounting'::text))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.has_role(auth.uid(), 'accountant'::public.app_role) OR public.can_edit_module(auth.uid(), 'accounting'::text)));


--
-- Name: asset_allocations Admins and inventory editors can delete allocations; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and inventory editors can delete allocations" ON public.asset_allocations FOR DELETE USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text)));


--
-- Name: goods_receipt_notes Admins and inventory editors can manage GRN; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and inventory editors can manage GRN" ON public.goods_receipt_notes USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text)));


--
-- Name: grn_items Admins and inventory editors can manage GRN items; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and inventory editors can manage GRN items" ON public.grn_items USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text)));


--
-- Name: asset_master_data Admins and inventory editors can manage asset master data; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and inventory editors can manage asset master data" ON public.asset_master_data USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text)));


--
-- Name: brands Admins and inventory editors can manage brands; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and inventory editors can manage brands" ON public.brands USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text)));


--
-- Name: product_categories Admins and inventory editors can manage categories; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and inventory editors can manage categories" ON public.product_categories USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text)));


--
-- Name: depreciation_schedules Admins and inventory editors can manage depreciation schedules; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and inventory editors can manage depreciation schedules" ON public.depreciation_schedules USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text)));


--
-- Name: asset_disposals Admins and inventory editors can manage disposals; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and inventory editors can manage disposals" ON public.asset_disposals USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text)));


--
-- Name: handover_slips Admins and inventory editors can manage handover slips; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and inventory editors can manage handover slips" ON public.handover_slips USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text)));


--
-- Name: inventory_items Admins and inventory editors can manage inventory items; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and inventory editors can manage inventory items" ON public.inventory_items USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text)));


--
-- Name: maintenance_records Admins and inventory editors can manage maintenance records; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and inventory editors can manage maintenance records" ON public.maintenance_records USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text)));


--
-- Name: product_groups Admins and inventory editors can manage product groups; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and inventory editors can manage product groups" ON public.product_groups USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'inventory'::text)));


--
-- Name: org_chart_positions Admins can manage org chart positions; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins can manage org chart positions" ON public.org_chart_positions USING (public.has_role(auth.uid(), 'admin'::public.app_role)) WITH CHECK (public.has_role(auth.uid(), 'admin'::public.app_role));


--
-- Name: organization_charts Admins can manage org charts; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins can manage org charts" ON public.organization_charts USING (public.has_role(auth.uid(), 'admin'::public.app_role)) WITH CHECK (public.has_role(auth.uid(), 'admin'::public.app_role));
...
--
-- Name: asset_allocations Authenticated users can create allocations; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Authenticated users can create allocations" ON public.asset_allocations FOR INSERT TO authenticated WITH CHECK (true);
...
--
-- Name: asset_allocations Authenticated users can update allocations; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Authenticated users can update allocations" ON public.asset_allocations FOR UPDATE TO authenticated USING (true);
...
--
-- Name: accounting_transactions Finance and project members can view transactions; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Finance and project members can view transactions" ON public.accounting_transactions FOR SELECT USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.has_role(auth.uid(), 'accountant'::public.app_role) OR ((project_id IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM (public.projects p
     LEFT JOIN public.team_members tm ON ((p.id = tm.project_id)))
  WHERE ((accounting_transactions.project_id = p.id) AND ((p.created_by = auth.uid()) OR (tm.user_id = auth.uid()))))))));
...
--
-- Name: projects Project creators, admins and project editors can delete project; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Project creators, admins and project editors can delete project" ON public.projects FOR DELETE USING (((auth.uid() = created_by) OR public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'projects'::text)));


--
-- Name: projects Project creators, admins and project editors can update project; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Project creators, admins and project editors can update project" ON public.projects FOR UPDATE USING (((auth.uid() = created_by) OR public.has_role(auth.uid(), 'admin'::public.app_role) OR public.has_role(auth.uid(), 'project_manager'::public.app_role) OR public.can_edit_module(auth.uid(), 'projects'::text)));


--
-- Name: materials Project owners and project editors can manage materials; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Project owners and project editors can manage materials" ON public.materials USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'projects'::text) OR (EXISTS ( SELECT 1
   FROM public.projects
  WHERE ((projects.id = materials.project_id) AND (projects.created_by = auth.uid())))))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'projects'::text) OR (EXISTS ( SELECT 1
   FROM public.projects
  WHERE ((projects.id = materials.project_id) AND (projects.created_by = auth.uid()))))));


--
-- Name: project_kpis Project owners and project editors can manage project KPIs; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Project owners and project editors can manage project KPIs" ON public.project_kpis USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'projects'::text) OR (EXISTS ( SELECT 1
   FROM public.projects
  WHERE ((projects.id = project_kpis.project_id) AND (projects.created_by = auth.uid())))))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'projects'::text) OR (EXISTS ( SELECT 1
   FROM public.projects
  WHERE ((projects.id = project_kpis.project_id) AND (projects.created_by = auth.uid()))))));


--
-- Name: project_items Project owners and project editors can manage project items; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Project owners and project editors can manage project items" ON public.project_items USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'projects'::text) OR (EXISTS ( SELECT 1
   FROM public.projects
  WHERE ((projects.id = project_items.project_id) AND (projects.created_by = auth.uid())))))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'projects'::text) OR (EXISTS ( SELECT 1
   FROM public.projects
  WHERE ((projects.id = project_items.project_id) AND (projects.created_by = auth.uid()))))));


--
-- Name: client_requirements Project owners and project editors can manage requirements; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Project owners and project editors can manage requirements" ON public.client_requirements USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'projects'::text) OR (EXISTS ( SELECT 1
   FROM public.projects
  WHERE ((projects.id = client_requirements.project_id) AND (projects.created_by = auth.uid())))))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'projects'::text) OR (EXISTS ( SELECT 1
   FROM public.projects
  WHERE ((projects.id = client_requirements.project_id) AND (projects.created_by = auth.uid()))))));


--
-- Name: team_members Project owners and project editors can manage team members; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Project owners and project editors can manage team members" ON public.team_members USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'projects'::text) OR public.is_project_owner(auth.uid(), project_id))) WITH CHECK ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_edit_module(auth.uid(), 'projects'::text) OR public.is_project_owner(auth.uid(), project_id)));
...
--
-- Name: tasks Task creators, project owners and project editors can delete ta; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Task creators, project owners and project editors can delete ta" ON public.tasks FOR DELETE USING (((auth.uid() = created_by) OR public.has_role(auth.uid(), 'admin'::public.app_role) OR public.user_can_access_project(auth.uid(), project_id) OR public.can_edit_module(auth.uid(), 'projects'::text)));


--
-- Name: tasks Task creators, project owners and project editors can update ta; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Task creators, project owners and project editors can update ta" ON public.tasks FOR UPDATE USING (((auth.uid() = created_by) OR public.has_role(auth.uid(), 'admin'::public.app_role) OR public.user_can_access_project(auth.uid(), project_id) OR public.can_edit_module(auth.uid(), 'projects'::text)));
...
--
-- Name: org_chart_positions Users can view org chart positions; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Users can view org chart positions" ON public.org_chart_positions FOR SELECT USING (true);


--
-- Name: organization_charts Users can view org charts; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Users can view org charts" ON public.organization_charts FOR SELECT USING (true);
...
--
-- Name: grn_items Users with inventory access can view GRN items; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Users with inventory access can view GRN items" ON public.grn_items FOR SELECT TO authenticated USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_view_module(auth.uid(), 'inventory'::text)));


--
-- Name: goods_receipt_notes Users with inventory access can view GRNs; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Users with inventory access can view GRNs" ON public.goods_receipt_notes FOR SELECT TO authenticated USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_view_module(auth.uid(), 'inventory'::text)));


--
-- Name: asset_allocations Users with inventory access can view allocations; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Users with inventory access can view allocations" ON public.asset_allocations FOR SELECT TO authenticated USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_view_module(auth.uid(), 'inventory'::text)));


--
-- Name: asset_master_data Users with inventory access can view assets; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Users with inventory access can view assets" ON public.asset_master_data FOR SELECT TO authenticated USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_view_module(auth.uid(), 'inventory'::text)));


--
-- Name: brands Users with inventory access can view brands; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Users with inventory access can view brands" ON public.brands FOR SELECT TO authenticated USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_view_module(auth.uid(), 'inventory'::text)));


--
-- Name: product_categories Users with inventory access can view categories; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Users with inventory access can view categories" ON public.product_categories FOR SELECT TO authenticated USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_view_module(auth.uid(), 'inventory'::text)));


--
-- Name: depreciation_schedules Users with inventory access can view depreciation schedules; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Users with inventory access can view depreciation schedules" ON public.depreciation_schedules FOR SELECT TO authenticated USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_view_module(auth.uid(), 'inventory'::text)));


--
-- Name: asset_disposals Users with inventory access can view disposals; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Users with inventory access can view disposals" ON public.asset_disposals FOR SELECT TO authenticated USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_view_module(auth.uid(), 'inventory'::text)));


--
-- Name: handover_slips Users with inventory access can view handover slips; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Users with inventory access can view handover slips" ON public.handover_slips FOR SELECT TO authenticated USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_view_module(auth.uid(), 'inventory'::text)));


--
-- Name: inventory_items Users with inventory access can view inventory items; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Users with inventory access can view inventory items" ON public.inventory_items FOR SELECT TO authenticated USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_view_module(auth.uid(), 'inventory'::text)));


--
-- Name: asset_location_history Users with inventory access can view location history; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Users with inventory access can view location history" ON public.asset_location_history FOR SELECT TO authenticated USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_view_module(auth.uid(), 'inventory'::text)));


--
-- Name: maintenance_records Users with inventory access can view maintenance records; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Users with inventory access can view maintenance records" ON public.maintenance_records FOR SELECT TO authenticated USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_view_module(auth.uid(), 'inventory'::text)));


--
-- Name: product_groups Users with inventory access can view product groups; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Users with inventory access can view product groups" ON public.product_groups FOR SELECT TO authenticated USING ((public.has_role(auth.uid(), 'admin'::public.app_role) OR public.can_view_module(auth.uid(), 'inventory'::text)));
...
--
-- Name: org_chart_positions; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.org_chart_positions ENABLE ROW LEVEL SECURITY;

--
-- Name: organization_charts; Type: ROW SECURITY; Schema: public; Owner: -
--
